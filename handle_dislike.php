...
try {
    ...
    $check_sql = "SELECT id FROM dislikes WHERE absensi_id = ? AND guru_id = ?";
    ...
    if ($result->num_rows > 0) {
        // Hapus (undislike)
        $delete_sql = "DELETE FROM dislikes WHERE absensi_id = ? AND guru_id = ?";
        ...
        // Tambahkan (dislike)
        $insert_sql = "INSERT INTO dislikes (absensi_id, guru_id) VALUES (?, ?)";
    ...
    $count_sql = "SELECT COUNT(*) as total FROM dislikes WHERE absensi_id = ?";
    $stmt_count = $conn->prepare($count_sql);
    $stmt_count->bind_param("i", $absensi_id);
    $stmt_count->execute();
    $count_result = $stmt_count->get_result()->fetch_assoc();
    $new_total_dislikes = $count_result['total'];
    $stmt_count->close();

    http_response_code(200);
    echo json_encode(['status' => $status, 'total_dislikes' => $new_total_dislikes]);
